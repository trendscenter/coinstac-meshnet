...
def end_remote(args):
    ...
    state_ = args["state"]
    URLS = list(set([input_list[site]["URL"] for site in input_list]))
    generateOutput(os.path.join(args['state']["outputDirectory"],'index.html'), len(args['input'].keys()))
    shutil.move(
        os.path.join(args['state']["outputDirectory"],'index.html'),
        os.path.join(args['state']["transferDirectory"],'index.html'),
    )
